...
# Define parameters
THREADS=2
ROWS=(99999) # the arguments for numRows, always 1 less than wanted.
#Rows getestete in Express bei latency: 100, 1000, 10'000, 100'000, 500'000, 1'000'000
#Datasize getestet in Express: 1'000, 4'000, 16'000, 32'000, 64'000 (also bis 64KB) Throughput nur 1 und 32KB 
DATASIZE=64000
...
for ROW in "${ROWS[@]}"; do
    ...
    # Wait a bit so serverB starts up before serverA
    sleep 2

    # Start serverA in its own directory
    (
        cd "$CODE_DIR/serverA" || exit
        ./serverA localhost:4442 $THREADS 0 $ROW $DATASIZE > /dev/null 2>&1 &
    )

    # Wait again to ensure servers are ready
    sleep 2
